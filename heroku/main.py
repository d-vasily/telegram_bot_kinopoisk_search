...
if not (exists('users.vdb')):
  ...
  #                             'rating_count', 'link', 'audio', 'subtitles', 'description', 'users'])
    ...
    set_info('genre', '| аниме| мистика| комедия| криминал| приключения| семейный| триллер| ужасы| фантастика| фэнтези| 1')
...
def delete_films_numbers(message):
    ...
    if len(list_num) == 0:
        ...
        my_films(message)
        return
    film_list = [film_list[i] for i in range(len(film_list)) if i not in list_num]
...
def start_search(message):
    ...
    markup.add(itembtn1, itembtn2, itembtn4, itembtn5, itembtn6)
...
def end_search(message):
    set_info(str(message.chat.id) + 'temp_films', '')
    menu(message)
...
def next_ten(message):
    if get_info(str(message.chat.id) + 'state') != 'search_in_process':
        bot.send_message(message.chat.id, "Search is not in process")
        return
    ...
    set_info(str(message.chat.id) + 'temp_films', '|'.join(list(df_ten['name'])))
...
def genre(message):
    ...
    bot.send_message(message.chat.id, 'Choose one variant:\n1 - all, 2 - anime, 3 - mystery,\n 4 - comedy, \
5 - crime, 6 - adventure,\n 7 - family, 8 - thriller, 9 - horror,\n 10 - sci-fi, 11 - fantasy', reply_markup=markup)
...
url_bot = 'https://kinopoisk-search.herokuapp.com/'
